...
현재 TaillogToss 프로젝트는 **문서/기획 100% 완료, 코드 0줄** 상태.
...
**보안 체크리스트:**
- [ ] login-with-toss: Rate-limit + Nonce + 연속실패 차단 + 요청 서명 + PII 로그 금지
- [ ] verify-iap-order: 멱등키 + 서킷브레이커 + 5xx만 재시도(최대2회, 지수백오프)
- [ ] send-smart-message: 빈도 제한 + noti_history 기록
- [ ] grant-toss-points: key 1회 사용 제한 + 에러코드 분기

### 검증
- mock 모드로 Edge Function 단독 호출 성공
- piiGuard: PII 포함 로그 시도 시 필터링 확인
- rateLimiter: 초과 요청 시 429 반환 확인
...
Day 7~    Phase 11 (Edge Function+보안) + Phase 12 (DB 마이그레이션) 병렬
