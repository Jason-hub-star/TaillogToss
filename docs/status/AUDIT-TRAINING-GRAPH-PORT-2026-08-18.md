...
> **2026-08-18 갱신**: A2 가 플랜 #2·#4·#6 을 흔들어 **2차 수렴**을 돌렸다(수렴 SSOT §10, 0.618 → 0.831).
> **계획 수준에서 닫힌 것: A1(린트 신설) · A2(게이트 재작성) · A4(번들 반전).** 구현은 아직 0.
> 아래 `[~]` = 플랜에 반영됨 / `[ ]` = 미해결.
>
> **2026-08-18 2차 갱신 — 실사용자 0(미출시) 확인**: 「기존 진행 유저」 페르소나가 전제한 마이그레이션 손실이 **현재는 성립하지 않는다**(출시 후엔 성립). 남는 실효 과제는 **A1 · A3 · A6 · A7** 넷.

## P0

- [~] **A1 [안전] 마킹 = UTI 감별 문구 누락** — *P7 #7 콘텐츠 안전 린트로 계획 반영. 구현 필요*
...
- [~] **A2 [명세 오류] 안전 게이트가 존재하지 않는 필드를 가리킨다** — *P7 #6 재작성: 런타임 게이트가 아니라 **빌드타임 필터**(`how[].authored`). 게이트 코드가 앱에서 사라진다*
...
- [~] **A4 [회귀] 오프라인 동작이 깨진다** — *P7 #2 반전: 노출 부분그래프가 **409KB**(gzip 102KB)라 번들 가능. 서버 이동을 철회해 오프라인 유지*
