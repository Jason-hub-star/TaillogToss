...
**핵심 아키텍처 결정**: SKILL.md Section 8의 **Toss + Supabase 통합 패턴**을 채택합니다.
...
**WebView 선택 근거** (React Native 불채택):
...
- **User 모델 변경** (SKILL.md Section 8 DB Contract 준수):
  ...
  ```
  - **마이그레이션 전략: Big-bang** (프로덕션 유저 미존재 시):
...
**Phase 2+ 백로그** (v1 출시 후 진행):

| 기능 | Phase | 변경 수준 | 설명 | 상태 |
|------|:-----:|-----------|------|------|
| 세그먼트 리텐션 | 2+ | **세그먼트+Smart Message** | 3일 미기록, 스트릭 직전, 행동 급증 자동 알림 | 백로그 |
| 공유 리워드 | 2+ | **바이럴 루프** | 친구 초대→조건부 포인트 보상 | 백로그 |
| 트레이너 인증 | 2-3 | **토스 인증** | 검증된 훈련사 매칭 | 백로그 |
| 인앱 광고 | 2+ | **토스 인앱 광고** | 무료 티어 수익화 | 백로그 |
...
- **Supabase 세션 토큰**: `supabase.auth.setSession()`으로 클라이언트에 저장 (Supabase SDK 기본 동작, httpOnly cookie or localStorage)

**연결해제/탈퇴 데이터 보존 정책** (B 보강):
- **UNLINK (사용자 직접 해제)**:
  - 즉시: `public.users.toss_user_key` → NULL 처리 (재연결 가능)
  - 30일 유예: 행동 로그/AI 코칭 데이터 보존 → 30일 후 자동 삭제 (배치 스크립트)
  - Supabase Auth 계정: 유지 (다른 로그인 수단 추가 가능성 대비)
- **WITHDRAWAL_TERMS (동의 철회)**:
  - 즉시: PII 삭제 (name, phone, email) → 익명화 (`user_xxxxx`)
  - 30일 유예: 나머지 데이터 삭제
- **WITHDRAWAL_TOSS (토스 탈퇴)**:
  - 즉시: 전체 데이터 삭제 (CASCADE) + Supabase Auth 계정 삭제
  - 법적 보존 의무 데이터: 결제 이력 5년 보존 (전자상거래법)
- **관리자 `remove-by-user-key`**: UNLINK와 동일 정책 적용
- **구현**: `referrer` 값에 따라 분기 → 삭제/익명화/유예 처리 + `noti_history`에 처리 로그 기록

**login-with-toss 무인증 엔드포인트 방어 정책** (verify_jwt=false 악용 방지):
- **Rate-limit**: IP당 분당 10회, 초과 시 429 반환 (Edge Function 내부 또는 앞단 CDN/WAF)
- **authorizationCode 1회 사용**: Toss 자체 제공 (10분 TTL, 재사용 시 `invalid_grant`)
- **Nonce/Idempotency**: Edge Function에서 authorizationCode를 멱등키로 사용 → DB 기록 → 동일 코드 재호출 시 첫 결과 반환
- **연속 실패 차단**: 동일 IP에서 연속 5회 실패 시 해당 IP 15분 차단
- **요청 서명**: FE에서 `timestamp + authorizationCode`의 HMAC-SHA256 서명 전달 → Edge Function에서 검증 (리플레이 방지, 5분 TTL)
...
- **빈도 제한/쿨다운 정책** (D 보강 — QA 심사 + 운영 기준):
  - 동일 templateSetCode: 사용자당 **10분 1회 / 하루 3회** (초과 시 큐잉, 다음 슬롯에 발송)
  - 행동기록 리마인더: **밤 10시~오전 8시 발송 금지** (사용자 timezone 기준)
  - 프로모션 메시지: **주 2회** 상한 (이탈 방지)
  - 긴급 메시지 (행동 급증 알림): 쿨다운 예외 허용, 단 **하루 1회** 상한
...
- **권한/상태별 라우팅 규칙** (E 보강 — 버그 방지):
  - `quick-log` 진입 시: 온보딩 미완료 → 온보딩 화면으로 리다이렉트 / 반려견 미등록 → 반려견 등록 화면으로
  - `daily-coach` 진입 시: 온보딩 미완료 → 온보딩 / AI 코칭 미생성 → 대시보드 (코칭 요청 유도)
  - `training-today` 진입 시: 온보딩 미완료 → 온보딩 / 훈련 시작 전 → 훈련 아카데미 메인
  - PRO 전용 화면 진입 시: 무료 유저 → PRO 업셀 바텀시트 (`useBottomSheet`) 표시
  - 공통: 모든 딥엔트리는 `authGuard` → `onboardingGuard` → `featureGuard` 순서로 미들웨어 체인 통과
- **AC**: 딥엔트리 탭 → 해당 화면 1초 이내 진입, 파라미터 프리필 정상 동작, 미완료 상태 시 적절한 리다이렉트 확인
...
### 섹션 10: 보안 & 개인정보
- **mTLS**: 인증서를 Base64 인코딩 → **Supabase Edge Function secrets**에 저장 (`TOSS_CLIENT_CERT_BASE64`, `TOSS_CLIENT_KEY_BASE64`)
...
- **Cursor**: `.cursor/mcp.json`에 `{ "mcpServers": { "apps-in-toss": { "command": "ax", "args": ["mcp", "start"] } } }`
...
- **Edge Function verify_jwt 분리**: `login-with-toss`만 `false` (유일한 예외), 나머지 전부 `true`
...
**권장 스킬화 우선순위**:
1. 세그먼트 + 분석 (마케팅 타겟팅 핵심)
2. Sentry 연동 (운영 모니터링)
3. 공유 리워드 (바이럴 성장)
4. Firebase 연동 (Push 보조 채널)
...
4. MCP 서버 설정 (`scoop install ax` → `claude mcp add`)
5. CI/CD 파이프라인 구성
6. IAP 테스트 상품 등록 + Smart Message 템플릿 생성
7. **토스 포인트 테스트 프로모션 코드로 get-key → execute → result 1회 성공 확인** (스크린샷/로그 첨부) — 실서비스 전 필수 선행 조건

### Phase 1: 인증 & 데이터 레이어 (2-3주)
1. **Supabase Edge Function `login-with-toss` 구현** (핵심):
   ...
   - `supabase/config.toml`에 `verify_jwt = false` 설정
2. FE에서 `appLogin()` → Edge Function 호출 → `supabase.auth.setSession()` 구현
3. DB 마이그레이션: `users` 테이블에 `toss_user_key TEXT UNIQUE NOT NULL` + RLS 정책
4. FastAPI: 기존 Supabase JWT 검증 유지 (변경 최소화)
5. Toss 연결해제 콜백 웹훅 엔드포인트 구현 (UNLINK/WITHDRAWAL_TERMS/WITHDRAWAL_TOSS 분기 처리)
6. **토큰 갱신 전략 구현**: Supabase refresh로 앱 세션 유지 + Toss refresh-token API 래퍼 (프로필 재조회 시)

### Phase 2: UI → TDS 마이그레이션 (3-5주)
1. 레이아웃 컴포넌트 (Top, BottomCTA, PageLayout)
2. 온보딩 설문 → TDS
3. 대시보드 → TDS (ListRow, Badge, 차트)
4. 행동 기록 폼 → TDS (TextField, useBottomSheet)
5. 분석 페이지 → TDS (차트, 타임라인)
6. AI 코칭 인터페이스 → TDS (Bubble, ListRow)
7. 훈련 아카데미 → TDS
8. 반려견 프로필, 설정 → TDS

### Phase 3: 결제 & 수익화 (5-6주)
1. IAP 상품 목록 UI + 구매 플로우 (SDK 1.2.2+)
2. **Edge Function `verify-iap-order`**: mTLS로 주문 상태 검증
3. `completeProductGrant()` + 앱 시작 시 `getPendingOrders()` 복구 로직
4. Sandbox 필수 테스트 3가지 (성공, 성공+서버실패, 에러)
5. BE 구독 상태 관리 + `toss_orders` 테이블

### Phase 4: 알림 & 마케팅 (6-7주)
1. **Edge Function `send-smart-message`**: mTLS로 Smart Message API 호출
2. **Edge Function `grant-toss-points`**: mTLS로 포인트 3-step 지급
3. 템플릿 코드 등록 (Console) + context 변수 매핑
4. 세그먼트 설정 (Console) + 프로모션 캠페인
5. Sentry 연동 (토스 미니앱 컨텍스트 태깅)

### Phase 5: 테스트 & QA 심사 (7-9주)
1. 단위/통합/E2E 테스트 완성
2. Sandbox App + QR 테스트
3. UX Writing 준수 검토
4. Toss QA 심사 제출 (2-3 영업일)
...
1. **Toss Login 상세** — OAuth2 full flow (generate-token, login-me, refresh-token, logout, 연결해제 콜백, PII 복호화 AES-256-GCM)
2. **Toss IAP 상세** — SDK 메서드 5개, 주문 상태 6가지, Sandbox 테스트 3시나리오, 버전 요구사항
3. **Smart Message 상세** — 5채널 (Push/Inbox/SMS/Alimtalk/Friendtalk), 템플릿+context, 응답 구조
4. **토스 포인트 상세** — 3-step key 기반, 에러코드 5종, 중복방지, 예산관리
5. **세그먼트/분석** — Console 타겟팅, 이벤트 로그 대시보드
6. **Sentry/Firebase 연동** — 모니터링 통합 패턴

## 검증 방법

1. **명세서 완성도**: PRD 템플릿 17개 섹션 + 부록 4개 (A~D) 모두 작성 확인
2. **기술 정합성**: SKILL.md Section 1-8 (특히 Section 8 Toss+Supabase 패턴)의 모든 요구사항이 명세서에 반영됐는지 확인
3. **기존 기능 커버리지**: DogCoach의 모든 기능(14개 테이블, 20+ API, 10개 페이지)이 마이그레이션 맵에 포함됐는지 확인
4. **CLAUDE.md 가이드라인**: 기존 개발 원칙이 토스 맥락으로 확장/리팩토링됐는지 확인
5. **Toss S2S API 정합성**: Login, IAP, Smart Message, Points 각 API의 엔드포인트/파라미터/에러코드가 명세서에 정확히 반영됐는지
6. **Edge Function 아키텍처**: mTLS 처리가 모두 Supabase Edge Function으로 분리됐는지 (FastAPI에 mTLS 코드 없음 확인)
7. **MCP 서버 동작**: `ax mcp start` → `docs-search` 도구로 TDS 컴포넌트, API 레퍼런스 검색 가능 확인
