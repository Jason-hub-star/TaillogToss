...
|---|---|---|---|---|---|---|
| `page-login-upgrade` | `/login` | `src/pages/login.tsx`, `src/lib/api/auth.ts`, `src/lib/hooks/useAuth.ts` | `docs/status/PROJECT-STATUS.md` | `docs/status/11-FEATURE-PARITY-MATRIX.md`, `docs/ref/AIT-SDK-2X-MIGRATION.md` | `feature-form-validation-and-submit`, `feature-error-and-retry-state` | auth guard pass, error states clear |
...
| — | `/onboarding/stage3-form` | `src/pages/onboarding/stage3-form.tsx`, `Backend/app/features/onboarding/*`, `Backend/app/features/coaching/prompts.py` | `docs/status/PROJECT-STATUS.md`, `docs/status/11-FEATURE-PARITY-MATRIX.md` | `docs/weekly/2026-W20.md` | `feature-form-validation-and-submit`, `feature-data-binding-and-loading` | Pro 핵심 3섹션 저장/복원, 선택 섹션 optional, AI 프롬프트 반영 |
...
| `page-settings-subscription-upgrade` | `/settings/subscription` | `src/pages/settings/subscription.tsx`, `src/lib/hooks/useSubscription.ts` | `docs/status/11-FEATURE-PARITY-MATRIX.md`, `docs/status/MISSING-AND-UNIMPLEMENTED.md` | `docs/ref/PRD-TailLog-Toss.md`, `docs/ref/AIT-IAP-MESSAGE-POINTS-REFERENCE.md` | `feature-data-binding-and-loading`, `feature-error-and-retry-state` | IAP state and recovery |
...
| `toss-mock-auth-ops` | Mock auth stability, stable userKey, and survey loop diagnosis | `docs/weekly/2026-W19.md`, `docs/status/PROJECT-STATUS.md` | stable userKey and route loop evidence captured |
...
| `toss-iap-proxy-ops` | IAP proxy, service role JWT detection, and subscriptions E2E recovery | `docs/weekly/2026-W19.md`, `docs/ref/AIT-IAP-MESSAGE-POINTS-REFERENCE.md` | subscription active state and proxy path verified |
...
| `toss-ait-build-ops` | `.ait` 빌드/업로드/standalone private launch 패턴, env/runtime setup 검증, `brand.icon` HTTPS URL 우선 규칙, host error escalation | `docs/weekly/2026-W19.md`, `docs/ref/AIT-PUBLISHING-READINESS.md`, `docs/status/PROJECT-STATUS.md` | bundle scan: Supabase marker + no local/data URI `brandIcon`; preferred `brandIcon:"https://"`; if Metro-off launch fails before JS marker, collect deploymentId/CLI URL/UI text/logcat and escalate |
| `toss-dev-server` | Metro, FastAPI, and adb reverse local server startup | `docs/status/PROJECT-STATUS.md`, `docs/weekly/2026-W19.md` | 8081/8765 health and adb reverse verified |
...
| `toss-sandbox-metro` | Toss Sandbox device, Metro entry scheme, and device QA | `docs/weekly/2026-W19.md`, `docs/ref/AIT-PUBLISHING-READINESS.md` | device launch and network path verified |
...
| `toss-iap-edge-recovery` | `verify-iap-order` 401/403 diagnosis and safe recovery | `docs/ref/AIT-IAP-MESSAGE-POINTS-REFERENCE.md`, `docs/weekly/2026-W19.md` | auth failure path separated from IAP success path |
