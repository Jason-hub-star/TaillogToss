...
Last Updated: 2026-05-13 (KST) — AI-TRAIN-001 플라이휠 1차 하드닝 완료. `coaching_synthetic_log.coaching_ids` ORM 타입을 migration의 `UUID[]`와 맞춰 `ARRAY(UUID(as_uuid=True))`로 정렬했고, Pro 심화 필드(`technique`, `steps`, `success_criteria`, `stop_criteria`, `reference_curriculum_ids`)를 품질점수에 반영했다. 관리자 검수 API `POST /api/v1/coaching/admin/training-candidates/{coaching_id}/review`를 추가해 `training_approved` 승인/반려를 SQL 직접수정 없이 처리할 수 있게 했다. `.claude/automations`의 daily synthetic 스케줄(08:00), weekly review(금요일 10:00), FastAPI 포트(8765), 현재 프로젝트 경로를 정렬했다. Targeted backend tests PASS(20 tests). 이전 Pro 심화 schema/OpenAI/ADB 검증도 유지.
...
| AI-TRAIN-001 | 훈련 데이터 플라이휠 | InProgress | AI 코칭 저장 → 구조화 필드 기반 품질 태깅 → 후보 플래그 → 관리자 review API 승인/반려 → approved JSONL export 골격. 2026-05-13 하드닝: `coaching_synthetic_log.coaching_ids` ORM↔DB UUID[] 정렬, `.claude/automations` 스케줄/포트/경로 정합성 보정 | 실제 daily synthetic 재실행 persistence 확인, 전문가 승인 UI, `src/lib/data/candidates/ai-coaching` 연결, action별 behavior matching 고도화, pg_cron/실행환경 등록, Fine-tune job 생성 |
