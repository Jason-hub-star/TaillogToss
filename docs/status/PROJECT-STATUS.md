...
Last Updated: 2026-05-13 (KST) — 자동화 오케스트레이션 정리 완료. 기본 실행은 `morning`, `AI data`, `nightly`, `weekly` 4개 오케스트레이터로 제한하고, 개별 TASK 프롬프트는 하위 작업으로 정렬했다. `taillog-ai-data-orchestrator`는 synthetic gen → Telegram review 순서로 AI-TRAIN-001 루프를 묶는다. `skills-web-enrichment-7day`는 앱 런타임 훈련데이터를 바꿀 수 있어 수동/DRY_RUN 전용으로 내렸고, nightly vision-labeling은 스키마/권한 검증 전까지 기본 실행에서 제외했다. 이전: AI-TRAIN-001 텔레그램 검수 재료 수집 v1 추가. 기존 플라이휠 하드닝(`coaching_synthetic_log.coaching_ids` UUID[] 정렬, 구조화 품질점수, 관리자 review API)에 이어, 자동화가 쓸 수 있는 `GET /api/v1/coaching/admin/training-candidates` 후보 목록과 `candidate-payload` API를 추가했다.
