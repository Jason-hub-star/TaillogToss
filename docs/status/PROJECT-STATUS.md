...
Last Updated: 2026-05-11 (KST) — 개발환경 즉시표시 1차 개선 완료 후 AIT private 성능 측정본 업로드/실행 확인 완료, DEV_LOCAL route data marker 3페이지 sweep 완료. React Query persistence를 user-scoped AsyncStorage로 추가하고 auth/session·subscription/orders/IAP·org entitlement는 persist 제외, logout/SIGNED_OUT 시 cache clear 처리. `queryPolicy`/`queryKeys` 통일, Auth bootstrap dogs 조회 공유, 일반 page guard의 subscription blocking 제거, 주요 페이지 shell/cached-first 렌더링, FastAPI logs `?limit=` + FE dashboard 20/analysis 200/default 100 적용. post-paint bootstrap deferral과 `[PERF][startup]` marker를 추가했고, AIT uploaded dashboard는 `loadingStart→first_paint_boundary=1163ms`, `js_entry→first_paint_boundary=294ms`. DEV_LOCAL에서 `/dashboard`, `/training/academy`, `/dog/profile`에 `page_shell_ready`/`page_cached_data_ready`/`page_fresh_data_settled` marker를 추가해 재측정: dashboard cached `1965ms` / fresh `8267ms`, training cached `2096ms` / fresh `8174ms`, dog profile cached `1835ms` / fresh `2340ms`. `Invalid semver: ''` render error는 `No current activity` 후 AppsInToss native constants가 비어 발생한 Sandbox transient로 재분류했고 clean restart 후 재현 없음. 다음 병목 후보는 dashboard/training fresh fetch 분해(FastAPI/Supabase fallback/behavior analytics/app-state refetch overlap). `npm run typecheck` PASS, `npm run test:app -- --runInBand --passWithNoTests` PASS(16 suites/100 tests), `cd Backend && venv/bin/pytest tests/ -v` PASS(48 tests).
