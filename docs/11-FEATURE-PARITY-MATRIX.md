...
|---|---|---|---|---|---|---|---|---|
| AUTH-001 | Auth | Supabase OAuth + guest | Toss Login bridge + Supabase session | rewrite | In Progress | integration/e2e | High | `[x] login.tsx mock` `[x] AuthContext + useAuth` `[x] authGuard + onboardingGuard` `[x] usePageGuard 15개 페이지 적용` `[x] login-with-toss Edge Function(mock)` `[x] rateLimiter/piiGuard/pepperRotation` `[ ] mTLS 인증서` `[ ] Toss Sandbox E2E` |
| APP-001 | App Shell | Next.js App Router | RN file-based routing app shell | rewrite | In Progress | unit/manual | Medium | `[x] 17라우트 + _app.tsx 프로바이더` `[x] 레이아웃 5종` `[x] 딥엔트리 3종` `[x] SurveyContext` `[ ] Sandbox 앱 검증` |
| UI-001 | Design System | Tailwind/Radix/Framer Motion | TDS React Native | rewrite | In Progress | visual/manual | High | `[x] TDS-ext 6 + shared 6 + features 22 = 34컴포넌트` `[x] 17개 화면 전체 빌드` `[x] tsc 통과` `[ ] 실기기 비주얼 QA` |
...
| IAP-001 | Billing | Stripe placeholder | Toss IAP + verify-iap-order | rewrite | In Progress | integration/e2e | High | `[x] 구독 화면 Card Stack (PRO+토큰+비교표)` `[x] useIsPro + usePurchaseIAP + useRestoreSubscription` `[x] featureGuard (PRO + 멀티독)` `[x] verify-iap-order Edge Function(mock)` `[x] grant-toss-points Edge Function(mock)` `[ ] 결제 E2E` |
