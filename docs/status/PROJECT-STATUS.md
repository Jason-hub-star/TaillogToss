...
Last Updated: 2026-05-13 (KST) — AI 코칭 6블록 유지 + Pro 심화 optional schema 확장 완료. `action_plan.items[]`/`next_7_days.days[]`에 기법·심리원리·도구·환경·성공/멈춤 기준·Plan B/C·상담지 근거·참고 커리큘럼 ID를 추가했고, `/src/lib/data` 커리큘럼을 축약한 backend reference catalog + 최대 3개 retrieval을 프롬프트에 연결했다. 사용자 표시 문장은 Toss UX Writing 톤(쉽고 짧은 해요체, 능동형/긍정형)으로 조정하고, Pro 접힘 영역 라벨은 `방법/이유/준비물/장소/멈출 신호`로 단순화했다. 실제 OpenAI 우디+5 fixture 호출 PASS(라벨 누출 0, 필수 구조 필드 채움; Case E 자원보호 우선순위 보정 후 `impulse_control` 반영 확인). ADB 실기기 `/coaching/result` 렌더 PASS, 기존 저장 리포트는 optional 필드 부재로 접힘 영역 미노출. `npm run typecheck` PASS, `npm run test:app -- --runInBand --passWithNoTests` PASS(16 suites/103 tests), `Backend/venv/bin/pytest Backend/tests/ -v` PASS(65 tests).
