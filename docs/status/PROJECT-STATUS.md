...
Latest Addendum: 2026-05-20 (KST) — `/dog/profile` production Toss AIT photo QA is now green on the connected Android device. Latest dev QA build `deploymentId=019e4409-85c4-7585-ae37-5de5f08384d2` launched with Metro off in `viva.republica.toss`, opened the OS Photos permission sheet, then Android Photo Picker, returned a selected image preview, and saved successfully. DB proof: `storage.objects` created `dog-profiles/2732a53d-8608-41c2-b9c1-1bec70e59f98/1212be92-6ece-41ac-a6b9-a9f04bd6b24c-1779258464646.jpg`, and `dogs.profile_image_url` now points to that public `dog-profiles` URL. Ops skill docs were aligned so DEV_LOCAL photo QA requires `photos/read` plus `adb reverse tcp:8081`, `tcp:5173`, and `tcp:8765`.

Latest Addendum: 2026-05-20 (KST) — `/dog/profile` DEV_LOCAL photo upload is now green. The earlier Sandbox “gallery impossible” note is superseded for the current setup: Android dev entry requires all three reverse ports (`8081`, `5173`, `8765`) plus `photos/read` in Granite config. `viva.republica.toss.test` entered `/dog/profile`, Android Photo Picker opened, a real device image returned to the avatar preview, and profile save completed after fixing React Native image body handling plus `dog-profiles` Storage RLS. New migration `20260520000100_dog_profiles_storage_policies.sql` was applied live. Age save was verified from UI (`3 -> 4` produced `birth_date=2022-05-20`) and test data was restored to `2023-03-15`. Validation: `npx tsc --noEmit` PASS, `DogPhotoPicker.test.tsx` 5 PASS.

Latest Addendum: 2026-05-20 (KST) — UI navigation consistency pass: `/dashboard/analysis` PNG back icon was promoted to reusable `src/components/shared/BackButton.tsx`, exported through `components/shared`, and applied to shared layouts plus direct header pages (`/dashboard/quick-log`, legal terms/privacy, dog add/profile, ops dog-add/settings, parent reports). `npx tsc --noEmit` and scoped `git diff --check` PASS. ADB launch to `/dashboard/analysis` reached Toss host but was blocked by `지금은 서비스를 사용할 수 없어요`, so app-internal visual QA remains pending. Daily evidence: `docs/daily/05-20/page-dashboard-analysis.md`.

Last Updated: 2026-05-20 (KST) — AIT coaching-limit hotfix was rebuilt, uploaded, and launched on device with Metro off. Latest AIT PASS: `deploymentId=019e42f0-841c-7952-a119-b44c58e35bee`, artifact `taillog-app-019e42f0-841c-7952-a119-b44c58e35bee.ait`, SHA256 `248280593fefde92d7f6dbb53ecf9cffba78720943dcd560ec64b6a0d1c7fa6f`, RN 0.84/0.72 both `0 errors / 0 warnings`. Bundle scan PASS: production Railway URL present, Supabase markers present, HTTPS Toss brand icon present, local/data brand icon leaks `0`, `ait-ad-test-*` `0`, `isDevToolsEnabled()` returns false, secret leak scan for service role/AIT/Toss keys `not_found`, and new copy/fallback markers present (`AI 코칭 하루 10회`, `상담지 기반 정밀 코칭`, `하루 1회 + 토큰 충전`, `결제 복원`, `PRO로 더 정밀하게 보기`, `isPro ? 10 : 1`). AIT upload PASS with private URL `intoss-private://taillog-app?_deploymentId=019e42f0-841c-7952-a119-b44c58e35bee`; ADB Metro-off launch PASS in actual Toss app with `Bundle loading completed successfully`, `Running "shared"`, and dashboard render proof at `/tmp/taillog-qa/ait-019e42f0-dashboard.png`. Remaining blocker: production Railway backend is still unavailable (`/health` -> `404 Application not found`), and local Railway CLI cannot deploy because OAuth refresh/link/token are unavailable; FastAPI-backed calls log `BACKEND_404` until Railway is restored and redeployed with `FREE_DAILY_COACHING_LIMIT=1`.

Previous: FREE AI coaching policy updated from 3/day to 1/day. Backend `FREE_DAILY_COACHING_LIMIT` is now 1, token-pack extension still adds up to 7 extra daily calls, and UI fallbacks/limit banners/subscription comparison all show `하루 1회 + 토큰 충전`. `/settings/subscription` copy now uses `결제 복원` with clearer restore alerts, and PRO value copy emphasizes `상담지 기반 정밀 코칭` instead of implying unlimited coaching. Coaching result free-user CTA now explains the product boundary: free coaching uses general survey + behavior logs, while PRO adds the Stage 3 consultation intake for 7-day plan, risk-signal, and expert-question blocks. Validation: `npx tsc --noEmit`, `Backend/venv/bin/pytest Backend/tests/test_routers.py Backend/tests/test_models.py -q`, backend py_compile, scoped `git diff --check`, and ADB `/settings/subscription` text dump PASS (`AI 코칭 하루 10회`, `상담지 기반 정밀 코칭`, `하루 1회 + 토큰 충전`, `결제 복원`). Previous: Subscription pricing UI hotfix applied. `IAP_PRODUCTS` centralizes console supply price, user-facing sale price, list price, promotion label, and token unit count, so future price swaps can be made from one catalog. `/settings/subscription` and `ProUpgradeSheet` show PRO `₩10,000 → ₩7,920/월`, token 10 `₩4,000 → ₩2,750`, token 30 `₩10,000 → ₩6,600`; ADB UI dump confirmed the text on device. FREE users can buy AI token packs without PRO, and backend decrements `ai_tokens_remaining` by 1 only when a FREE user successfully generates coaching beyond the free daily allowance.

Previous: 개발환경 즉시표시 1차 개선 완료 후 AIT private 성능 측정본 업로드/실행 확인 완료, DEV_LOCAL route data marker 3페이지 sweep 완료. React Query persistence를 user-scoped AsyncStorage로 추가하고 auth/session·subscription/orders/IAP·org entitlement는 persist 제외, logout/SIGNED_OUT 시 cache clear 처리. `queryPolicy`/`queryKeys` 통일, Auth bootstrap dogs 조회 공유, 일반 page guard의 subscription blocking 제거, 주요 페이지 shell/cached-first 렌더링, FastAPI logs `?limit=` + FE dashboard 20/analysis 200/default 100 적용. post-paint bootstrap deferral과 `[PERF][startup]` marker를 추가했고, AIT uploaded dashboard는 `loadingStart→first_paint_boundary=1163ms`, `js_entry→first_paint_boundary=294ms`. DEV_LOCAL에서 `/dashboard`, `/training/academy`, `/dog/profile`에 `page_shell_ready`/`page_cached_data_ready`/`page_fresh_data_settled` marker를 추가해 재측정: dashboard cached `1965ms` / fresh `8267ms`, training cached `2096ms` / fresh `8174ms`, dog profile cached `1835ms` / fresh `2340ms`. 추가 query/API marker로 느린 구간을 분해해 dashboard backend aggregate `6355ms`, training progress/feedback/behavior analytics backend `5701/5823/6246ms`를 확인했고, dashboard/training stale 정책을 완화해 cached re-entry에서 backend refetch 없이 dashboard `fromJsStartMs=215`, training `fromJsStartMs=420`에 cached/fresh-settled 동시 완료. 추가로 FastAPI dashboard DB 왕복을 5→2로 축소하고, training progress/feedback 동시 조회는 같은 backend rows 요청을 공유하도록 보강했으며, behavior analytics는 전체 `BehaviorLog` 객체 조회 대신 SQL aggregate + memo-only 조회로 전환. Railway production `f704fc3f...` 배포 후 새 AIT `019e162c-c1b7-7a29-a3e0-df4f21cf162a`로 `/training/academy`를 재측정했을 때 `api_training_behavior_analytics_backend_done=7266ms`였고, 추가 server timing 배포 `da40cc90...` + AIT `019e163c-d6fe-7168-9b81-2c784e043966`에서는 `api_training_behavior_analytics_backend_done=4509ms`, `Server-Timing total_ms=1954.5ms`로 분리됨. ownership path split 배포 `318e4e14...` 후 같은 AIT에서 `b2c_match=true`, `ownership_path=b2c_owner`, `dog_lookup_ms=4033.1ms`, `aggregate_ms=290.4ms`로 확인되어 B2B fallback이 아니라 첫 `dogs` row lookup/DB wake가 주요 병목으로 재분류됨. `Invalid semver: ''` render error는 `No current activity` 후 AppsInToss native constants가 비어 발생한 Sandbox transient로 재분류했고 clean restart 후 재현 없음. `npm run typecheck` PASS, `npm run test:app -- --runInBand --passWithNoTests` PASS(16 suites/101 tests), `Backend/venv/bin/pytest Backend/tests/ -v` PASS(55 tests).

Latest DB latency check: live DB `dogs.id` lookup is not the bottleneck. `dogs_pkey` exists as a unique btree primary-key index, `public.dogs` was only `17` rows / `96 kB`, and live `EXPLAIN (ANALYZE, BUFFERS)` for the measured dog used `Index Scan using dogs_pkey` with `Execution Time: 0.021ms`. New local psql connections to the Railway-provided Supabase pooler URL completed the indexed query in about `0.12s-0.21s`, while Railway deployment `318e4e14...` has `sleepApplication=true` and warm public `/health` calls were still about `0.48s-0.80s`. Because `Backend/app/core/database.py` uses Supabase pooler `NullPool` + `statement_cache_size=0`, the first `db.execute()` in `/behavior-analytics` pays connection/open cost inside `dog_lookup_ms`. Next safest optimization is operational first: disable Railway sleep or keep one warm replica, then evaluate a small app-level asyncpg pool while preserving `statement_cache_size=0`.

Railway sleep-off follow-up: config-as-code `sleepApplication=false` deploy `1e5b9f79...` failed after image push with empty deploy logs, matching earlier failed `sleep=false` attempts in deployment history. The setting was reverted and backend redeployed successfully as `22f9b5ed...` with `sleepApplication=true`. Public `/health` from sleeping state took `5.291s`; warm checks after restore were about `0.646s-0.846s`. AIT remeasurement is blocked until the Android device is visible to `adb devices`.

AIT warm remeasurement: after ADB reconnect (`R3CXB0QH0LY`) and `/health` keep-warm, the same AIT `019e163c...` launched directly to `/training/academy` in production Toss with Metro off. `first_paint_boundary=973ms`, `page_shell_ready=2281ms`, `api_training_behavior_analytics_backend_done=5385ms`, `page_fresh_data_settled=7704ms`. Server timing improved from prior `dog_lookup_ms=4033.1ms` to `3156.4ms`, but dog lookup remains dominant (`ownership_path=b2c_owner`, `aggregate_ms=277.9ms`, `total_ms=3435.3ms`). Small asyncpg pool experiment is implemented locally: Supabase pooler keeps `statement_cache_size=0` and uses `pool_size=5`, `max_overflow=5`, `pool_timeout=10`, `pool_recycle=300` instead of `NullPool`; backend tests PASS (57). Railway deployment for this experiment is pending because free-tier deploys to `us-west2` are blocked during peak hours.

Railway/FastAPI vs Supabase Edge migration scan completed. Heavy folders were excluded (`node_modules`, `.git`, `Backend/venv`, build/cache artifacts). Dedicated tree snapshot added at `docs/ref/REPOSITORY-TREE.md`; migration audit added at `docs/ref/RAILWAY-EDGE-FUNCTION-MIGRATION-AUDIT.md`. Result: Railway is still a broad FastAPI surface, so full removal is not safe as a first move. Best first Edge migration candidate is `/api/v1/dogs/{dog_id}/behavior-analytics`, followed by dashboard aggregate and training rows read if real-device Edge networking proves stable.

Previous: DogPhotoPicker 개발/실행 모드 분리 완료. 과거 “사진 업로드 성공” 기록은 실제 개발용 Sandbox 앨범 선택 성공이 아니라 `dog-profiles` Storage 업로드 + `dogs.profile_image_url` 표시 성공으로 재분류한다. `viva.republica.toss.test`는 Android manifest에 `READ_MEDIA_*`/`READ_EXTERNAL_STORAGE`가 없어 `fetchAlbumPhotos`가 `NO_PERMISSION`을 반환하므로 개발모드에서는 실제 갤러리 선택 검증이 불가하다. 개발 중에는 `테스트 사진 사용` fallback으로 data URI 업로드/저장 흐름을 검증하고, 실제 사진 선택은 새 `.ait` 빌드 업로드 후 실제 Toss 앱(`viva.republica.toss`)에서 최종 확인한다. `npm run typecheck` PASS, `DogPhotoPicker.test.tsx` 3 PASS, Sandbox `/dog/profile` fallback Alert 확인 완료.

Previous: AIT private standalone 실행 재확인 완료. 최신 업로드본 `019e14a7-ca44-7f9b-bd58-a9be19376360`을 Metro off 상태에서 실제 Toss 앱 activity(`viva.republica.toss/.intoss.MiniAppSchemeActivity`)로 실행했고, `viva.republica.toss/im.toss.rn.granite.core.GraniteActivity`까지 진입해 온보딩 화면(`테일로그`, `반려견 행동, 30초면 기록 끝`, `토스로 시작하기`)이 정상 렌더됨. logcat에 `Bundle loading completed successfully`, `Running "shared"`, `[AIT-BUILD] taillog-appsintoss-wrapper-20260511-0958-cli`가 찍혔고 `loadJSBundleFromMetro()`/`RNCSafeAreaProvider`/`Invariant`는 없음. 이전 `지금은 서비스를 사용할 수 없어요`는 앱 코드 크래시가 아니라 Toss 앱 로그인 계정과 Apps in Toss 워크스페이스 멤버 매핑 문제로 재분류한다. 현재 관리자 계정 `gmdqn2tp@gmail.com` 로그인 후 standalone gate PASS.

Previous: IAP/Ads/Smart Message 실기기 QA 진행: IAP Sandbox 3시나리오 false-success/loading 잔여 버그 수정 후 새 AIT에서 실패 Alert + 버튼 복귀 확인. Smart Message는 current user 대상 HTTP 200 + `noti_history.success=true` 실발송 확인. Ads는 최신 업로드 AIT `019e00dd-24bb-7fa1-b385-251e67eae2e8`로 B1/B2/B3를 순차 재시도했고, 세 슬롯 모두 mock fallback 없이 live adGroupId로 실제 SDK 호출 후 `ad_error` 상세 payload를 확보함: 공통 `code=1007`, `domain=@apps-in-toss/framework`, `This feature is not supported in the current environment`. IAP backend hardcoded 재점검 결과 release 빌드는 env가 없어도 Railway public URL로 고정하고, `127.0.0.1:8765`은 DEV local Metro host에서만 사용하도록 제한했으며 legacy `verifyIAPOrder()`도 404/408 시 FastAPI proxy로 우회하도록 보강. Railway 재배포 성공: deployment `d11ae99d-c5bf-47ba-98a7-0bea0a635848` SUCCESS, `/health` HTTP 200, IAP proxy route smoke HTTP 401로 라우트 생존 확인. `verify-iap-order` Edge v13은 Toss IAP 검증을 `/order/get-order-status` + `x-toss-user-key`로 교체해 재배포 완료. 업로드 AIT `019e00f0...` 성공 재시도에서 SDK 결제 이벤트가 서버 grant 완료보다 먼저 와 앱이 `GRANT_FAILED`로 조기 확정하는 프론트 상태머신 버그를 추가 확인했고, `createOneTimePurchaseOrder`를 서버 grant 완료 후 `GRANT_COMPLETED` 처리하도록 수정. 공식 문서 기준 `_app.tsx` 엔트리를 `AppsInToss.registerApp(AppContainer, { context })`로 교체하고 최신 AIT `019e01b9-3c4c-7677-b6b9-d80529a2d868`를 API 키 기반 `ait deploy --scheme-only`로 신규 배포 성공, hash `8a18b7d9dbb1e0c8be8ccec2513246f5e2f00cfb30f772e26f3ae84d923a7707`, 번들에 `AppsInTossInitialProps`/`RNNavigationBar`/`TDSProvider` 포함 확인, `brandIcon:"https://static.toss.im/..."`, local path/data URI 없음, `ait-ad-test-*` 0, `tsc`/문법검사 통과. Metro on control은 같은 private URL로 대시보드 진입 성공했고 logcat에 `loadJSBundleFromMetro()` + `[AIT-BUILD] taillog-appsintoss-wrapper-20260507-1745`가 찍혀 app JS/공식 wrapper 정상임을 확인. 최신 Android 샌드박스 APK(`rn-miniapp-real-release-protected.apk`, 2026-04-22) 재설치 후 `viva.republica.toss.test lastUpdateTime=2026-05-07 18:38:12` 확인했지만 Metro-off private launch는 여전히 JS marker 없이 host error(`앱 실행도중 문제가 발생했습니다.`)로 종료. 실제 Toss app `5.258.1` 직접 private scheme은 `지금은 서비스를 사용할 수 없어요.`로 종료. 샌드박스 홈에는 `taillog-app`이 노출되므로 앱 목록/로그인 자체는 통과. outdated sandbox app/app JS/registerApp/brand.icon/API deploy는 소거됐고, 남은 1순위는 direct adb scheme과 콘솔 QR/test-button 컨텍스트 차이 또는 Apps in Toss host의 deployment entitlement/lookup 문제다. 최소 smoke AIT도 생성 완료: current `taillog-app.ait`, embedded deploymentId `019e01f5-9ae6-774d-90d7-e68ac7132db5`, hash `552c233d3d5d3ada1eee5676f66fc6950e667b65664713d2e72c060b6ed81d03`, marker `[AIT-SMOKE] minimal-standalone-20260507-1845`; 콘솔 업로드/QR 실행으로 host vs app-bundle 문제를 최종 분리해야 함.
