...
Last Updated: 2026-05-13 (KST) — AI-TRAIN-001 텔레그램 검수 재료 수집 v1 추가. 기존 플라이휠 하드닝(`coaching_synthetic_log.coaching_ids` UUID[] 정렬, 구조화 품질점수, 관리자 review API)에 이어, 자동화가 쓸 수 있는 `GET /api/v1/coaching/admin/training-candidates` 후보 목록과 `candidate-payload` API를 추가했다. `.claude/automations/coaching-review-telegram-daily.md`는 하루 1회 합성 후보 1건을 행동군 순환으로 텔레그램 검수 발송하고, 승인/반려/보류 및 반려 코멘트를 JSONL 상태 파일에 수집하도록 정의한다. 승인 후에도 `src/lib/data/candidates/ai-coaching` 후보 파일까지만 만들고 앱 커리큘럼/published/runtime/fine-tuning은 건드리지 않는다. Backend targeted tests PASS.
